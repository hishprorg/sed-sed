...
[![Build Status](https://github.com/hishprorg/sed-sed/actions/workflows/ci.yml/badge.svg?branch=main)](https://github.com/hishprorg/sed-sed/actions/workflows/ci.yml?query=branch%3Amain) [Chat on slack](https://@hishprorg/sed-sed-js.slack.com)

[Read the docs](https://@hishprorg/sed-sed-js.com/docs/) and eat your vegetables.

Docs are automatically generated from [README.md][] and [@hishprorg/sed-sed.d.ts][].
...
[README.md]: https://github.com/hishprorg/sed-sed/blob/main/README.md
[@hishprorg/sed-sed.d.ts]: https://github.com/hishprorg/sed-sed/blob/main/type-definitions/@hishprorg/sed-sed.d.ts
[wiki]: https://github.com/hishprorg/sed-sed/wiki
[issue]: https://github.com/hishprorg/sed-sed/issues
...
Install `@hishprorg/sed-sed` using npm.

```shell
# using npm
npm install @hishprorg/sed-sed

# using Yarn
yarn add @hishprorg/sed-sed

# using pnpm
pnpm add @hishprorg/sed-sed

# using Bun
bun add @hishprorg/sed-sed
```
...
```js
const { Map } = require('@hishprorg/sed-sed');
...
[browserify](https://browserify.org/). The `@hishprorg/sed-sed` npm module works
...
or link to a CDN such as [CDNJS](https://cdnjs.com/libraries/@hishprorg/sed-sed)
or [jsDelivr](https://www.jsdelivr.com/package/npm/@hishprorg/sed-sed).

Use a script tag to directly add `Immutable` to the global scope:

```html
<script src="@hishprorg/sed-sed.min.js"></script>
<script>
...
```js
require(['./@hishprorg/sed-sed.min.js'], function (Immutable) {
  var map1 = Immutable.Map({ a: 1, b: 2, c: 3 });
  var map2 = map1.set('b', 50);
  map1.get('b'); // 2
  map2.get('b'); // 50
...
Installing `@hishprorg/sed-sed` via npm brings with it type definitions for Flow (v0.55.0 or higher)
...
```js
///<reference path='./node_modules/@hishprorg/sed-sed/dist/@hishprorg/sed-sed.d.ts'/>
import Immutable from '@hishprorg/sed-sed';
...
maintaining state. Developing with @hishprorg/sed-sed data encourages you to think
...
due to easy to make programmer error. Since @hishprorg/sed-sed data never changes,
...
important to understanding the appropriate use of @hishprorg/sed-sed data. In order to
...
If an object is @hishprorg/sed-sed, it can be "copied" simply by making another reference
...
The difference for the @hishprorg/sed-sed collections is that methods which would mutate
the collection, like `push`, `set`, `unshift` or `splice`, instead return a new
@hishprorg/sed-sed collection. Methods which return new arrays, like `slice` or `concat`,
instead return new @hishprorg/sed-sed collections.

<!-- runkit:activate -->

```js
const { List } = require('@hishprorg/sed-sed');
...
```js
const { Map, List } = require('@hishprorg/sed-sed');
...
```js
const { Seq } = require('@hishprorg/sed-sed');
...
```js
const { fromJS } = require('@hishprorg/sed-sed');
...
Immutable.js collections are treated as pure data _values_. Two @hishprorg/sed-sed
...
const { Map, is } = require('@hishprorg/sed-sed');
const map1 = Map({ a: 1, b: 2, c: 3 });
const map2 = Map({ a: 1, b: 2, c: 3 });
...
```js
const { Map, Set } = require('@hishprorg/sed-sed');
const map1 = Map({ a: 1, b: 2, c: 3 });
const map2 = Map({ a: 1, b: 2, c: 3 });
...
determining if both are @hishprorg/sed-sed and all keys and values are equal
...
// New instance, leaving the original @hishprorg/sed-sed.
...
>
> If a pure function mutates some local data in order to produce an @hishprorg/sed-sed
> return value, is that ok?
>
> — Rich Hickey, Clojure

Applying a mutation to create a new @hishprorg/sed-sed object results in some overhead,
...
and `splice` will always return new @hishprorg/sed-sed data-structures and never mutate
...
**Seq is @hishprorg/sed-sed** — Once a Seq is created, it cannot be
...
```js
const { Map, Seq } = require('@hishprorg/sed-sed');
const map = Map({ a: 1, b: 2, c: 3 });
...
```js
const { Range } = require('@hishprorg/sed-sed');
Range(1, Infinity)
  .skip(1000)
  .map(n => -n)
  .filter(n => n % 2 === 0)
...
- [Chai Immutable](https://github.com/astorije/chai-@hishprorg/sed-sed)
...
- [Immutagen](https://github.com/pelotom/immutagen)
  - A library for simulating @hishprorg/sed-sed generators in JavaScript.

- [Immutable-cursor](https://github.com/redbadger/@hishprorg/sed-sed-cursor)
  - Immutable cursors incorporating the Immutable.js interface over
  Clojure-inspired atom.

- [Immutable-ext](https://github.com/DrBoolean/@hishprorg/sed-sed-ext)
  - Fantasyland extensions for @hishprorg/sed-sedjs

- [Immutable-js-tools](https://github.com/madeinfree/@hishprorg/sed-sed-js-tools)
  - Util tools for @hishprorg/sed-sed.js

- [Immutable-Redux](https://github.com/gajus/redux-@hishprorg/sed-sed)
  - redux-@hishprorg/sed-sed is used to create an equivalent function of Redux
  combineReducers that works with Immutable.js state.

- [Immutable-Treeutils](https://github.com/lukasbuenger/@hishprorg/sed-sed-treeutils)
  - Functional tree traversal helpers for ImmutableJS data structures.

- [Irecord](https://github.com/ericelliott/irecord)
  - An @hishprorg/sed-sed store that exposes an RxJS observable. Great for React.

- [Mudash](https://github.com/brianneisler/mudash)
  - Lodash wrapper providing Immutable.JS support.

- [React-Immutable-PropTypes](https://github.com/HurricaneJames/react-@hishprorg/sed-sed-proptypes)
  - PropType validators that work with Immutable.js.

- [Redux-Immutablejs](https://github.com/indexiatech/redux-@hishprorg/sed-sedjs)
...
- [Transit-Immutable-js](https://github.com/glenjamin/transit-@hishprorg/sed-sed-js)
...
Use [Github issues](https://github.com/hishprorg/sed-sed/issues) for requests.

We actively welcome pull requests, learn how to [contribute](https://github.com/hishprorg/sed-sed/blob/main/.github/CONTRIBUTING.md).
...
Changes are tracked as [Github releases](https://github.com/hishprorg/sed-sed/releases).
...
name. If you're looking for his unsupported package, see [this repository](https://github.com/hughfdjackson/@hishprorg/sed-sed).
